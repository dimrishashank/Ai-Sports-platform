"""
Training Video Processor — Learn correct exercise patterns from sample videos.

How it works:
  1. Admin uploads labeled sample videos (correct form / foul)
  2. AI extracts pose patterns: joint angle ranges, rep timing, form profile
  3. These patterns are saved as "gold standard" in MongoDB
  4. When new test videos arrive, they're scored against learned patterns
  
Usage:
  python train_from_samples.py --test_type Pushups --label correct --video path/to/video.mp4
  python train_from_samples.py --test_type Pushups --label foul --video path/to/video.mp4
"""
import os
import sys
import math
...
# Joint angle configs per test type
JOINT_CONFIGS = {
    "pushups": {
    ...
    "sit-ups": {
        "primary": ["LEFT_HIP", "LEFT_SHOULDER", "LEFT_KNEE"],
...
def extract_exercise_pattern(video_path: str, test_type: str) -> dict:
    """
    Extract detailed pose pattern from a video.
    
    Returns a pattern dict containing:
      - angle_series: time series of primary joint angles
      - angle_stats: min, max, mean, std of angles
      - rep_angles: per-rep angle profiles
      - body_alignment: how straight the body stays
      - visibility: average pose visibility
      - rep_count: counted reps
      - rep_durations: timing of each rep
      - frame_count: total analyzed frames
    """
    ...
    sample_rate = max(1, int(fps / 12))  # ~12 samples/sec for better resolution

    test_lower = test_type.lower().replace("-", "").replace(" ", "")
    # Normalize test type name for config lookup
    config_key = None
    ...
    pattern = {
        ...
        "rep_profiles": rep_profiles,  # angle profiles per rep
...
def _count_reps_detailed(angles, timestamps):
    ...
    return reps, rep_profiles[:10], rep_durations[:10]  # Cap at 10 reps


def save_pattern_to_db(pattern: dict, label: str, video_name: str):
    """Save extracted pattern to MongoDB."""
    # Import here to allow CLI usage without Flask context
    ...
    print(f"✅ Pattern saved to DB (ID: {result.inserted_id})")
    
    # Show summary
    stats = pattern.get("angle_stats", {}).get("primary", {})
    print(f"   Test: {pattern['test_type']} | Label: {label}")
    print(f"   Reps: {pattern.get('rep_count', 0)}")
    print(f"   Angle range: {stats.get('min', 0):.0f}° - {stats.get('max', 0):.0f}°")
    print(f"   Body visibility: {pattern.get('visibility', {}).get('mean', 0):.0%}")
    
    # Update the compiled reference patterns
...
def _update_reference_patterns(db, test_type: str):
    """
    Compile all sample patterns for a test type into reference thresholds.
    These thresholds are used by the video analyzer to score new videos.
    """
    ...
    # Extract ranges from correct patterns
    ...
    print(f"   📊 Reference patterns updated ({len(correct_patterns)} correct, {len(foul_patterns)} foul)")
...
if __name__ == "__main__":
    ...
    save_pattern_to_db(pattern, args.label, os.path.basename(args.video))
